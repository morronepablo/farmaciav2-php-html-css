$(document).ready(function(){
                      ...
                      $('#error').text('Ingrese una fecha de vencimiento !');
                        ...
                        if(precio_compra == ''){
                          $('#error').text('Ingrese un precio de compra !');
                            ...
                            prods.push(producto);
                            PCompra = parseFloat(producto.precio_compra);
                            PCantidad = parseInt(producto.cantidad);
                            TTotal = TTotal + (PCompra * PCantidad);
                            $('#total').val(TTotal.toFixed(2));
                            let template = '';
                            template=`
                                <tr prodId="${producto.id}" prodPrecio="${producto.precio_compra}" prodCantidad="${producto.cantidad}">
    ...
    $(document).on('click','.borrar-producto',(e)=>{
        ...
        let precio_c = $(elemento).attr('prodprecio');
        let cantidad = $(elemento).attr('prodcantidad');
        PCompra = parseFloat(precio_c);
        ...
        if (Filas > 0){
            let TTotal = 0 ;
            prods.forEach(function(prod, index){
                PCompra = prod.precio_compra;
                PCantidad = prod.cantidad;
                TTotal = TTotal + (PCompra * PCantidad);
            })
            $('#total').val(TTotal.toFixed(2));
        }else{
        ...
        if(codigo == ''){
          ...
          if(fecha_compra == ''){
            ...
            if(fecha_entrega == ''){
              ...
              if(total == ''){
                  $('#error-compra').text('Ingrese un total de compra !');
                  $('#noadd-compra').hide('slow');
                  $('#noadd-compra').show(1000);
                  $('#noadd-compra').hide(2000);
              }else{
