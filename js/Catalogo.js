			...
			datatable = $('#lotes').DataTable( {
				...
				columnDefs:[{
						"render":function(data, type, row){
							let campo='';
							if(row.mes <= 3){
								campo = `<h1 class="badge badge-warning"> Cuidado </h1>`;
							}
							if(row.mes == 0 && row.dia <= 5){
								campo = `<h1 class="badge badge-secondary"> Por Vencer </h1>`;
							}
							if(row.mes == 0 && row.dia <= 0 && row.hora <=0){
