$(document).ready(function(){
				...
				if(response=='encontrado'){
					...
					$.post('../controlador/recuperar.php',{funcion,email,dni},(response2)=>{
						...
						if(response2=='enviado'){
							...
							$('#aviso1').show();
							$('#aviso1').text('Contraseña restablecida. Revise su correo.');
							$('#form-recuperar').trigger('reset');
						}
						else{
							Cerrar_Loader('error_envio');
							$('#aviso').show();
							$('#aviso').text('No se pudo restablecer la contraseña.');
							$('#form-recuperar').trigger('reset');
						}
					})
				}
	...
	function Cerrar_Loader(Mensaje){
			...
			switch (Mensaje) {
				...
				case 'error_envio':
						tipo = 'error';
						texto = "El correo no pudo envirse, porfavor intente de nuevo.";
						mostrar = true;
					break;
				case 'error_usuario':
						tipo = 'error';
						texto = "El usuario perteneciente a estos datos no fu encontrrado..";
						mostrar = true;
					break;
				default:
						swal.close();
