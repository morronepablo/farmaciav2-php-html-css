...
function getHtml($id_venta){
      ...
      <table>
        ...
        foreach ($venta_producto->objetos as $objeto) {
          ...
          $plantilla.='<tr>
            ...
            <td class="servic">$'.number_format($objeto->precio, 2, ',', '.').'</td>
            <td class="servic">$'.number_format($objeto->subtotal, 2, ',', '.').'</td>
          </tr>';
        }
        $calculos= new Venta();
        $calculos->buscar_id($id_venta);
        foreach ($calculos->objetos as $objeto) {
          $igv=$objeto->total*0.21;
    ...
    <footer>
      Creado por MorroneCoputación (Morrone Pablo Martín) Ingeniero Informatico y Analista desarrollador.
    </footer>
  </body>';
	return $plantilla;
}
